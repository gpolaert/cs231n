...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores_norm[np.arange(h1.shape[0]), y] -= 1
        scores_norm /= relu1.shape[0]

        dW2 = relu1.T.dot(scores_norm)
        dW2 += reg * W2

        db2 = np.sum(scores_norm, axis=0)

        dhidden = scores_norm.dot(W2.T)
        dhidden[h1 <= 0] = 0
    ...
    def predict(self, X):
        ...
        scores_norm = np.exp(scores) / np.exp(scores).sum(axis=1)[:, np.newaxis]

        y_pred = scores_norm.argmax(axis=1)
